...
export function StatCard({
  ...
  accentColor = "blue", prefix, suffix, loading = false, className,
}: StatCardProps) {
  if (loading) {
    return (
      <div className={cn("rounded-2xl border bg-white p-5 space-y-3 shadow-card", className)}>
  ...
  const trendColor = trend === undefined ? "" : trend > 0 ? "text-emerald-600" : trend < 0 ? "text-red-600" : "text-content-muted";
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        {icon && (
          <div className={cn("h-9 w-9 rounded-xl flex items-center justify-center", ICON_BG[accentColor])}>
            {icon}
          </div>
        )}
      </div>
      ...
      <div className="flex items-end gap-1 mb-1">
        {prefix && <span className="text-content-secondary text-lg mb-0.5">{prefix}</span>}
        <span className="text-3xl font-black text-content-primary tabular-nums animate-count-up">
          {typeof value === "number" ? value.toLocaleString("en-IN") : value}
        </span>
        {suffix && <span className="text-content-secondary text-lg mb-0.5">{suffix}</span>}
      </div>

      {subValue && <p className="text-content-muted text-xs">{subValue}</p>}

      {trend !== undefined && TrendIcon && (
        <div className={cn("flex items-center gap-1 mt-3 text-xs font-semibold", trendColor)}>
