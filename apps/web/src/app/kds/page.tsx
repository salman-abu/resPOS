...
import {
  ...
  LayoutGrid, Flame, Snowflake, Wine, Croissant,
  AlertTriangle, Volume2, VolumeX, BarChart3, ArrowRight,
...
const STATION_CFG: Record<string, {
  label: string; icon: React.ReactNode;
  bg: string; border: string; accent: string; header: string;
}> = {
  HOT_KITCHEN:  { label:'Hot Kitchen',  icon:<Flame      className="h-4 w-4"/>, bg:'bg-orange-950/60', border:'border-orange-800',   accent:'text-orange-400',   header:'bg-orange-900/70' },
  COLD_KITCHEN: { label:'Cold Kitchen', icon:<Snowflake   className="h-4 w-4"/>, bg:'bg-cyan-950/60',   border:'border-cyan-800',     accent:'text-cyan-400',     header:'bg-cyan-900/70'   },
  BAR:          { label:'Bar',          icon:<Wine        className="h-4 w-4"/>, bg:'bg-violet-950/60', border:'border-violet-800',   accent:'text-violet-400',   header:'bg-violet-900/70' },
  BAKERY:       { label:'Bakery',       icon:<Croissant   className="h-4 w-4"/>, bg:'bg-amber-950/60',  border:'border-amber-800',    accent:'text-amber-400',    header:'bg-amber-900/70'  },
};
...
function elapsed(ts: number): number { return Math.floor((Date.now() - ts) / 1000); }
...
function TicketCard({
  ticket, onBump, onToggleItem,
}: {
  ticket: KdsTicket;
  onBump: (id: string) => void;
  onToggleItem: (ticketId: string, itemId: string, currentDone: boolean) => void;
}) {
  const cfg = STATION_CFG[ticket.station] || STATION_CFG['HOT_KITCHEN'];
  const secs = elapsed(new Date(ticket.created_at).getTime());
  const doneCount = ticket.items.filter(i => i.status === 'READY' || i.status === 'SERVED').length;
  ...
  return (
    <div className={cn(
      ...
      'kds-card-enter',
      cfg.bg, cfg.border, urgencyBg(secs),
    )}>

      {/* Card Header */}
      <div className={cn('flex items-center justify-between px-3.5 py-2.5', cfg.header)}>
        <div className="flex items-center gap-2">
          {ticket.priority && (
            <AlertTriangle className="h-3.5 w-3.5 text-red-400 animate-pulse" />
          )}
          <span className={cn('font-black text-base', cfg.accent)}>
            {ticket.order_type === 'DINE_IN' ? `T-${ticket.table_number || '?'}` : ticket.order_type.slice(0,2)}
          </span>
          <span className="text-slate-400 text-xs font-mono">{ticket.order_id}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={cn('text-xs font-bold font-mono', urgencyClass(secs))}>
            ⏱ {fmtElapsed(secs)}
          </span>
          <span className={cn(
            'text-[10px] font-bold px-2 py-0.5 rounded-full',
            ticket.status === 'PREPARING' ? 'bg-amber-500/20 text-amber-400' : 
            ticket.status === 'READY' ? 'bg-emerald-500/20 text-emerald-400' :
            'bg-slate-700 text-slate-400'
          )}>
            {ticket.status === 'PREPARING' ? 'COOKING' : ticket.status === 'READY' ? 'READY' : 'NEW'}
          </span>
        </div>
      </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  ...
                  isDone ? 'bg-slate-700 text-slate-500' : 'bg-slate-600 text-slate-200'
                )}>
                  ×{item.quantity}
                </span>
              </div>
              {item.variant && <p className="text-slate-400 text-xs mt-0.5">{item.variant}</p>}
              {item.notes && (
      ...
      {ticket.items.length > 1 && (
        <div className="px-3.5 pb-2">
          <div className="flex items-center justify-between mb-1">
            <span className="text-slate-500 text-[10px]">{doneCount}/{ticket.items.length} items</span>
            <span className="text-slate-500 text-[10px]">{Math.round((doneCount/ticket.items.length)*100)}%</span>
          </div>
          <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
            <div
              className={cn('h-full rounded-full transition-all duration-500', allDone ? 'bg-emerald-500' : 'bg-amber-500')}
...
export default function KDSPage() {
  ...
  const token = typeof window !== 'undefined' ? localStorage.getItem('rpos_token') ?? undefined : undefined;

  // No live-clock interval needed - elapsed() reads Date.now() each render

  const playAlert = useCallback(() => {
    if (!muted) {
      try { const a = new Audio('data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAA...'); a.play().catch(()=>{}); } catch {}
    }
  }, [muted]);
  ...
  const { status: socketStatus } = useKdsSocket({
    ...
    onItemDone: useCallback((kotId, itemId, done) => {
      setTickets(prev => prev.map(t => {
        ...
        const items = t.items.map(i => i.id === itemId ? { ...i, status: done ? 'READY' : 'SENT_TO_KDS' } : i);
        return { ...t, items };
      }));
    }, []),
  ...
  const fetchActiveKots = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/kds/kots${station !== 'ALL' ? `?station=${station}` : ''}`, {
  ...
  const handleToggleItem = useCallback(async (ticketId: string, itemId: string, currentDone: boolean) => {
    const newDoneState = !currentDone;
    
    // Optimistic UI
    setTickets(prev => prev.map(t => {
      if (t.id !== ticketId) return t;
      const items = t.items.map(i => i.id === itemId ? { ...i, status: newDoneState ? 'READY' : 'SENT_TO_KDS' } : i);
      const anyDone = items.some(i => i.status === 'READY' || i.status === 'SERVED');
  ...
  const visible = tickets.filter(t =>
    station === 'ALL' || t.station === station
  );

  const pending  = tickets.filter(t => t.status !== 'READY').length;
  const avgSecs  = tickets.length
    ? Math.floor(tickets.reduce((s,t) => s + elapsed(new Date(t.created_at).getTime()), 0) / tickets.length)
    : 0;
  const oldest   = tickets.reduce((max,t) => Math.max(max, elapsed(new Date(t.created_at).getTime())), 0);

  const TABS: { key: Station; label: string; icon: React.ReactNode }[] = [
    { key:'ALL',         label:'All',         icon:<LayoutGrid className="h-3.5 w-3.5"/>      },
    { key:'HOT_KITCHEN', label:'Hot Kitchen', icon:<Flame className="h-3.5 w-3.5"/>           },
    { key:'COLD_KITCHEN',label:'Cold Kitchen',icon:<Snowflake className="h-3.5 w-3.5"/>       },
    { key:'BAR',         label:'Bar',         icon:<Wine className="h-3.5 w-3.5"/>            },
    { key:'BAKERY',      label:'Bakery',      icon:<Croissant className="h-3.5 w-3.5"/>       },
  ];
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 overflow-hidden">
      ...
      <header className="flex-shrink-0 flex items-center gap-4 px-5 py-3 bg-slate-900 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="hidden sm:block">
            <p className="text-white font-black text-sm leading-none">Kitchen Display</p>
          ...
          <div className="flex items-center gap-2">
            {socketStatus === 'connected' ? (
                ...
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider hidden sm:block">Connecting</span>
                </div>
            ) : (
        ...
        <div className="flex items-center gap-3 ml-2">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 border border-slate-700">
            <div className={cn('h-2 w-2 rounded-full', pending > 0 ? 'bg-orange-500 animate-pulse' : 'bg-emerald-500')} />
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 border border-slate-700">
            ...
            <span className={cn('font-bold text-sm', urgencyClass(avgSecs))}>{fmtElapsed(avgSecs)}</span>
          ...
          <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 border border-slate-700">
            <AlertTriangle className={cn('h-3.5 w-3.5', oldest > 1200 ? 'text-red-400' : 'text-slate-400')} />
            <span className={cn('font-bold text-sm', urgencyClass(oldest))}>{fmtElapsed(oldest)}</span>
        ...
        <div className="flex items-center gap-1.5 mx-auto">
          {TABS.map(({ key, label, icon }) => {
            const cnt = key === 'ALL' ? tickets.length : tickets.filter(t => t.station === key).length;
            return (
              <button
                key={key}
              ...
              >
                ...
                {cnt > 0 && (
                  <span className={cn(
                    ...
                    station === key ? 'bg-slate-900 text-white' : 'bg-slate-700 text-slate-300'
                  )}>
                    {cnt}
                  </span>
                )}
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">No pending tickets for this station</p>
          </div>
        </div>
      ) : (
            ...
            {visible.map((ticket) => (
              <div
                ...
                className={cn(
                  ...
                  bumped.includes(ticket.id) && 'opacity-0 scale-95 -translate-y-2'
                )}
              >
                <TicketCard
                  ticket={ticket}
      ...
      <div className="flex-shrink-0 flex items-center justify-center gap-6 px-5 py-2 bg-slate-900/80 border-t border-slate-800">
        ...
        <span className="text-slate-500 text-xs">Tap item to mark done · Tap BUMP when served</span>
      </div>
    </div>
  );
}
