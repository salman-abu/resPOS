...
export default function POSPage() {
  ...
  const { items: cartItems, order_type, table_number, pax_count,
          active_order_id, setOrderType, addItem, setActiveOrderId, clearCart } = useCartStore();
  const cartTotals = calcCartTotals(cartItems);

  // Rotate upsell nudge every 8s
  useEffect(() => {
    const id = setInterval(() => setNudgeIdx((i) => (i + 1) % UPSELL_NUDGES.length), 8000);
  ...
  const loadMenu = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (!isMenuCacheStale() && !force) {
        const [cats, its] = await Promise.all([getCachedCategories(), getCachedMenuItems()]);
  ...
  const displayedItems = useMemo(() => {
    let f = selectedCategoryId ? items.filter((i) => i.category_id === selectedCategoryId) : items;
    if (search.trim()) {
  ...
  const cartQtyMap = useMemo(() => {
    ...
    cartItems.forEach((ci) => { m[ci.item_id] = (m[ci.item_id] ?? 0) + ci.quantity; });
    return m;
  }, [cartItems]);

  const handleAddItem = useCallback((item: MenuItem) => { addItem(item); }, [addItem]);
  ...
  const handleFireKOT = async () => {
    if (!active_order_id) {
      const res = await fetch(`${API}/orders`, {
        ...
        body: JSON.stringify({
          order_type, table_id: useCartStore.getState().table_id, pax_count,
          items: cartItems.map((ci) => ({
            item_id: ci.item_id, variant_id: ci.variant_id,
            quantity: ci.quantity, unit_price: ci.unit_price,
            notes: ci.notes, course_number: ci.course_number,
          })),
        }),
      });
      ...
      await fetch(`${API}/orders/${order.id}/kot`, {
        method: 'POST',
        headers: { ...getAuthHeader(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ item_ids: order.order_items.map((oi: { id: string }) => oi.id) }),
      });
    }
  };

  // ─── Handle payment success ─────────────────────────────────────────────────
  const handlePaymentSuccess = useCallback(async (invoiceId: string) => {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex-shrink-0 flex items-center gap-3 px-4 py-3 bg-background-card border-b border-border">
          ...
          <div className="flex items-center gap-2.5 mr-1">
            ...
            <span className="text-content-primary font-black text-sm hidden md:block tracking-tight">resPOS</span>
          </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown className={cn("h-3 w-3 transition-transform", showOrderTypeMenu && "rotate-180")} />
            </button>
            {showOrderTypeMenu && (
              <div className="absolute top-full left-0 mt-1 w-36 bg-background-card border border-border rounded-xl shadow-elevated z-20 overflow-hidden animate-scale-in">
                {(['DINE_IN', 'TAKEAWAY', 'DELIVERY'] as const).map((ot) => (
                  <button
                    key={ot}
                    onClick={() => { setOrderType(ot); setShowOrderTypeMenu(false); }}
                    className={cn(
          ...
          {order_type === 'DINE_IN' && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface-2 border border-border">
              ...
              <span className="text-content-secondary text-sm font-medium">
                {table_number ? `Table ${table_number}` : <span className="text-amber-400">No Table</span>}
              </span>
              <span className="text-content-muted text-xs">· {pax_count} pax</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            <div className={cn("flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg",
            ...
            <button
              onClick={() => { setRefreshing(true); loadMenu(true); }}
            ...
            <a href="/dashboard" className="h-8 w-8 rounded-lg bg-surface-2 border border-border flex items-center justify-center text-content-muted hover:text-content-primary transition-colors" title="Dashboard">
        ...
        {showNudge && cartItems.length > 0 && (
          <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-amber-500/8 border-b border-amber-500/15 animate-fade-in">
            <span className="text-xs text-amber-300 font-medium flex-1">{UPSELL_NUDGES[nudgeIdx]}</span>
            <button onClick={() => setShowNudge(false)} className="text-amber-400/60 hover:text-amber-400 transition-colors">
              <X className="h-3.5 w-3.5" />
            </button>
          </div>
        )}
          ...
          <div className="flex-shrink-0 px-4 pt-3 pb-1 flex items-center gap-2">
            ...
            <span className="text-content-muted text-xs">
              ...
              {selectedCategoryId && categories.find(c => c.id === selectedCategoryId) && (
                <> in <span className="text-content-secondary font-medium">{categories.find(c => c.id === selectedCategoryId)?.name}</span></>
              )}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 pt-2 scrollbar-thin">
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {Array.from({ length: 15 }).map((_, i) => <ItemCardSkeleton key={i} />)}
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 py-20">
              ...
              <div className="text-center">
                <p className="text-content-primary font-semibold text-lg">No items found</p>
                <p className="text-content-muted text-sm mt-1">
                  {search ? `No results for "${search}"` : 'This category has no available items'}
                </p>
              </div>
              {search && (
                <button onClick={() => setSearch('')} className="text-brand-400 text-sm font-medium hover:underline">
                  Clear search
                </button>
              )}
            </div>
      ...
      {billPrintData && (
        <ThermalBill
          invoice={billPrintData.payments
            ? billPrintData  // already flat invoice
            : { ...billPrintData.invoice, payments: billPrintData.invoice?.payments ?? [] }
          }
          order={billPrintData.order ?? {
            order_type,
            pax_count,
            table_number,
            items: cartItems.map(ci => ({
