...
export default function TablesPage() {
  ...
  const handleTableClick = (table: TableData) => {
    if (table.status === 'AVAILABLE') {
      ...
      router.push(
        `/pos?table_id=${table.id}&table_number=${table.table_number}`,
      );
    } else if (
      ['OCCUPIED', 'BILLED'].includes(table.status) &&
      table.orders[0]
    ) {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              style={{
                background:
                  filter === 'ALL' ? 'var(--brand)' : 'var(--bg-elevated)',
            ...
            {Object.entries(STATUS_CFG).map(([s, cfg]) => (
              <button
                ...
                style={{
                  background:
                    filter === s ? cfg.dot + '33' : 'var(--bg-elevated)',
                  color: filter === s ? cfg.dot : 'var(--text-muted)',
                  border: `1px solid ${filter === s ? cfg.dot : 'var(--border)'}`,
                }}
              >
                <span
                ...
                return (
                  ...
                  >
                    ...
                    {activeOrder && (
                      <div
                        className="mt-2 pt-2"
                        style={{ borderTop: `1px solid ${cfg.border}44` }}
                      >
                        <div
