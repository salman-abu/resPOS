...
export default function SuperAdminTenants() {
  ...
  const impersonate = async (id: string) => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001";
      const token = localStorage.getItem("super_admin_token");
      const res = await fetch(`${apiUrl}/super-admin/tenants/${id}/impersonate`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-neutral-400 mt-1">Manage active subscriptions and perform support impersonations.</p>
        </div>
        <Button className="bg-red-600 hover:bg-red-700">Provision New Tenant</Button>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-neutral-800">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-8 text-center text-neutral-500">Loading tenants...</td>
                  </tr>
                ) : (
                  tenants.map((tenant) => (
                    <tr key={tenant.id} className="hover:bg-neutral-800/50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        <p className="text-xs text-neutral-500 mt-1">{tenant.slug}</p>
                      </td>
                      <td className="px-6 py-4">
                        <Badge variant="outline" className="bg-neutral-950 text-neutral-300">
                          {tenant.subscription_plan}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-neutral-300">{tenant._count.outlets}</td>
                      <td className="px-6 py-4 text-neutral-300">{tenant._count.users}</td>
                      <td className="px-6 py-4">
                        {tenant.is_active ? (
                          <Badge className="bg-green-500/10 text-green-500 border-green-500/20">Active</Badge>
                        ) : (
                          <Badge className="bg-red-500/10 text-red-500 border-red-500/20">Suspended</Badge>
                        )}
                      </td>
